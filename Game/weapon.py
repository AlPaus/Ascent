  ...
  def shoot(self):
    ...
    if (pygame.time.get_ticks()-self.last_shot >= self.rapidity and self.ammo > 0 and not self.reloading):
      if not self.effect_name:
        b = Bullet(self.game, self.rect.centerx, self.rect.centery, self.angle, self.bullet_path, self.damsage, self.bullet_speed, self.bullet_ratio, self.team, self, self.bullet_ttl)
      else:
